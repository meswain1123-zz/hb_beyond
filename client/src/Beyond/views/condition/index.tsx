...
class ConditionIndex extends Component<Props, State> {
  ...
  render() {
    if (this.state.loading) {
      ...
      const page_size = 7;
      const filtered: any[] = this.state.conditions ? this.state.conditions.filter(o => 
        ...
        <Grid container spacing={1} direction="column">
          <Grid item container spacing={1} direction="row">
            <Grid item xs={3}>
              ...
              <Tooltip title={`Create New Condition`}>
                <Fab size="small" color="primary" style={{marginLeft: "8px"}}
                  onClick={ () => {
                    this.setState({ redirectTo:`/beyond/condition/create` });
                  }}>
                  <Add/>
                </Fab>
              </Tooltip> 
                  ...
                  <Grid key={key} item container spacing={1} direction="row">
                    <Grid item xs={2}>
                      <Tooltip title={`View details for ${o.name}`}>
                        <Button 
                          ...
                          onClick={ () => {
                            this.setState({ redirectTo:`/beyond/condition/details/${o._id}` });
                          }}>
                            {o.name}
                        </Button>
                      </Tooltip>
                    ...
                    <Grid item xs={1}>
                      <Tooltip title={`Edit ${o.name}`}>
                        <Fab size="small" color="primary" style={{marginLeft: "8px"}}
                          onClick={ () => {
                            this.setState({ redirectTo:`/beyond/condition/edit/${o._id}` });
                          }}>
                          <Edit/>
                        </Fab>
                      </Tooltip> 
...
export default connector(ConditionIndex);
