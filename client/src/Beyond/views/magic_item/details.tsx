...
class MagicItemDetails extends Component<Props, State> {
  ...
  load_object(id: string) {
    this.setState({ loading: true }, () => {
      this.api.getFullObject("magic_item", id).then((res: ModelBase | null) => {
        if (res) {
          this.setState({ obj: (res as MagicItem).clone(), loading: false });
        }
      });
    });
  }
      ...
      return (
        <Grid container spacing={1} direction="column">
          <Grid item>
            <ObjectDetails 
              obj={this.state.obj}
              data_type="magic_item"
              type_label="Magic Items"
            />
          </Grid>
        </Grid>
      );
