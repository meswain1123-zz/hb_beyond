...
class GameClassIndex extends Component<Props, State> {
    ...
    if (this.state.import_checked.length > this.state.importing_pos) {
      this.setState({ importing: this.state.import_checked[this.state.importing_pos], importing_pos: this.state.importing_pos + 1 }, () => {
        ...
        this.api.get5eObjectFromAPI(this.state.importing).then((res: any) => {
          // Translate it into a GameClass object.
          const game_class_obj = new GameClass();
          game_class_obj.copy5e(res);
          if (game_class_obj.name !== "") {
  ...
  render() {
    if (this.state.loading) {
        ...
        <Grid container spacing={1} direction="column">
          <Grid item container spacing={1} direction="row">
            <Grid item xs={3}>
              ...
              <Tooltip title={`Create New Game Class`}>
                <Fab size="small" color="primary" style={{marginLeft: "8px"}}
                  onClick={ () => {
                    this.setState({ redirectTo:`/beyond/class/create` });
                  }}>
                  <Add/>
                </Fab>
              </Tooltip> 
              { true && <Tooltip title={`Import Game Classes`}>
                <Fab size="small" color="primary" style={{marginLeft: "8px"}}
                  onClick={ () => {
                    this.setState({ mode: "import" }, () => {
                      if (this.state.import_game_classes === null) {
                        this.api.get5eObjects("classes").then((res: any) => {
                          this.setState({ 
                            import_game_classes: res.results,
                            page_num: 0 
                          });
                        });
                      }
                  ...
                  <Grid key={key} item container spacing={1} direction="row">
                    <Grid item xs={2}>
                      <Tooltip title={`View details for ${o.name}`}>
                        <Button 
                          ...
                          onClick={ () => {
                            this.setState({ redirectTo:`/beyond/class/details/${o._id}` });
                          }}>
                            {o.name}
                        </Button>
                      </Tooltip>
                    ...
                    <Grid item xs={1}>
                      <Tooltip title={`Edit ${o.name}`}>
                        <Fab size="small" color="primary" style={{marginLeft: "8px"}}
                          onClick={ () => {
                            this.setState({ redirectTo:`/beyond/class/edit/${o._id}` });
                          }}>
                          <Edit/>
                        </Fab>
                      </Tooltip> 
      ...
      const page_size = 7;
      const filtered: any[] = this.state.import_game_classes ? this.state.import_game_classes.filter(o => 
        (this.state.start_letter === "" || o.name.toUpperCase().startsWith(this.state.start_letter)) && 
              ...
              <Grid item container spacing={1} direction="row">
                ...
                <Grid item xs={2}>
                  <Button 
                    fullWidth variant="contained" color="primary" 
                    onClick={ () => {
                      if (this.state.import_game_classes) {
                        const import_checked: string[] = [];
                        if (this.state.import_game_classes.length !== this.state.import_checked.length) {
                          this.state.import_game_classes.forEach(s => {
                            if (this.state.game_classes && !this.state.game_classes.some(s2 => s2.name === s.name)) {
                      ...
                      { (this.state.import_game_classes && this.state.import_game_classes.length === this.state.import_checked.length) ? "Deselect All" : "Select All" }
                  </Button>
                </Grid>
                <Grid item xs={4}>
                  Name
              ...
              { filtered_and_paged.map((o, key) => {
                return (
                  <Grid key={key} item container spacing={1} direction="row">
                    <Grid item xs={2}>
                      <Checkbox 
                        checked={ this.state.game_classes ? (this.state.game_classes.some(s => s.name === o.name) || (this.state.import_checked.some(s => s === o.url) && !this.state.import_checked.slice(this.state.importing_pos === 0 ? 0 : (this.state.importing_pos - 1)).some(s => s === o.url))) : false } 
                        disabled={true}
                        color="primary" 
                      />
                    </Grid>
...
export default connector(GameClassIndex);
