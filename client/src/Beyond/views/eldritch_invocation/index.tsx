...
import { EldritchInvocation } from "../../models";
// import { 
//   SCHOOLS 
...
const mapState = (state: RootState) => ({
  // eldritch_invocations: state.app.eldritch_invocations,
...
export interface State { 
  ...
  mode: string;
  import_eldritch_invocations: any[] | null;
...
class EldritchInvocationIndex extends Component<Props, State> {
  constructor(props: Props) {
    ...
    this.state = {
      ...
      search_string: "",
      mode: "index",
      import_eldritch_invocations: null,
      importing_pos: 0,
      import_checked: [],
      importing: "",
      page_num: 0,
    ...
    if (this.state.import_checked.length > this.state.importing_pos) {
      this.setState({ importing: this.state.import_checked[this.state.importing_pos], importing_pos: this.state.importing_pos + 1 }, () => {
        ...
        this.api.get5eObjectFromAPI(this.state.importing).then((res: any) => {
          // Translate it into a Eldritch Invocation object.
  ...
  render() {
    if (this.state.loading) {
      ...
      const page_size = 7;
      const filtered: any[] = this.state.eldritch_invocations ? this.state.eldritch_invocations.filter(o => 
        ...
        <Grid container spacing={1} direction="column">
          <Grid item container spacing={1} direction="row">
            <Grid item xs={3}>
              ...
              <Tooltip title={`Create New Eldritch Invocation`}>
                <Fab size="small" color="primary" style={{marginLeft: "8px"}}
                  onClick={ () => {
                    this.setState({ redirectTo:`/beyond/eldritch_invocation/create` });
                  }}>
                  <Add/>
                </Fab>
              </Tooltip> 
              { true && <Tooltip title={`Import Eldritch Invocations`}>
                <Fab size="small" color="primary" style={{marginLeft: "8px"}}
                  onClick={ () => {
                    this.setState({ mode: "import" }, () => {
                      if (this.state.import_eldritch_invocations === null) {
                        this.api.get5eObjects("features/choose-eldritch-invocations").then((res: any) => {
                          this.setState({ 
                            import_eldritch_invocations: res.choice.from,
                            page_num: 0 
                          });
                        });
                      }
                  ...
                  <Grid key={key} item container spacing={1} direction="row">
                    <Grid item xs={2}>
                      <Tooltip title={`View details for ${o.name}`}>
                        <Button 
                          ...
                          onClick={ () => {
                            this.setState({ redirectTo:`/beyond/eldritch_invocation/details/${o._id}` });
                          }}>
                            {o.name}
                        </Button>
                      </Tooltip>
                    ...
                    <Grid item xs={1}>
                      <Tooltip title={`Edit ${o.name}`}>
                        <Fab size="small" color="primary" style={{marginLeft: "8px"}}
                          onClick={ () => {
                            this.setState({ redirectTo:`/beyond/eldritch_invocation/edit/${o._id}` });
                          }}>
                          <Edit/>
                        </Fab>
                      </Tooltip> 
      ...
      const page_size = 7;
      const filtered: any[] = this.state.import_eldritch_invocations ? this.state.import_eldritch_invocations.filter(o => 
        (this.state.start_letter === "" || o.name.toUpperCase().startsWith(this.state.start_letter)) && 
      ...
      return (
        <Grid container spacing={1} direction="column">
          <Grid item container spacing={1} direction="row">
            <Grid item xs={3}>
              <span className={"MuiTypography-root MuiListItemText-primary header"}>
                Importing Eldritch Invocations
              </span>
              <Tooltip title={`Back`}>
                <Fab size="small" color="primary" style={{marginLeft: "8px"}}
                  onClick={ () => {
              ...
              <Grid item container spacing={1} direction="row">
                ...
                <Grid item xs={2}>
                  <Button 
                    fullWidth variant="contained" color="primary" 
                    onClick={ () => {
                      if (this.state.import_eldritch_invocations) {
                        const import_checked: string[] = [];
                        if (this.state.import_eldritch_invocations.length !== this.state.import_checked.length) {
                          this.state.import_eldritch_invocations.forEach(s => {
                            if (this.state.eldritch_invocations && !this.state.eldritch_invocations.some(s2 => s2.name === s.name)) {
                      ...
                      { (this.state.import_eldritch_invocations && this.state.import_eldritch_invocations.length === this.state.import_checked.length) ? "Deselect All" : "Select All" }
                  </Button>
                </Grid>
                <Grid item xs={4}>
                  Name
              ...
              { filtered_and_paged.map((o, key) => {
                return (
                  <Grid key={key} item container spacing={1} direction="row">
                    <Grid item xs={2}>
                      <Checkbox 
                        checked={ this.state.eldritch_invocations ? (this.state.eldritch_invocations.some(s => s.name === o.name) || (this.state.import_checked.some(s => s === o.url) && !this.state.import_checked.slice(this.state.importing_pos === 0 ? 0 : (this.state.importing_pos - 1)).some(s => s === o.url))) : false } 
                        disabled={true}
                        color="primary" 
                      />
                    </Grid>
...
export default connector(EldritchInvocationIndex);
