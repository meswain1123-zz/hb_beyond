...
const mapDispatch = {
  ...
  // addSense: (obj: Sense) => ({ type: 'ADD', dataType: 'senses', payload: obj })
}
...
class SenseDetails extends Component<Props, State> {
          ...
          <Grid container spacing={1} direction="column">
                  ...
                  onClick={ () => {
                    this.api.deleteObject(this.state.obj).then((res: any) => {
                      if (this.props.objects) {
                        this.props.setSenses(this.props.objects.filter(o => o._id !== this.state.obj._id));
                        this.setState({ redirectTo:`/beyond/sense` });
                      }
                    });
                  }}>
            ...
            <Grid item>
              ...
              <Tooltip title={`Edit ${this.state.obj.name}`}>
                <Fab size="small" color="primary" style={{marginLeft: "8px"}}
                  onClick={ () => {
                    this.setState({ redirectTo:`/beyond/sense/edit/${this.state.obj._id}` });
                  }}>
                  <Edit/>
                </Fab>
              </Tooltip> 
...
export default connector(SenseDetails);
