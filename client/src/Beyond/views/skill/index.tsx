...
class SkillIndex extends Component<Props, State> {
  ...
  render() {
    if (this.state.loading) {
      ...
      const page_size = 7;
      const filtered: any[] = this.state.skills ? this.state.skills.filter(o => 
        (this.state.ability_score === "ALL" || o.use_ability_score === this.state.ability_score) && 
        ...
        <Grid container spacing={1} direction="column">
          <Grid item container spacing={1} direction="row">
            <Grid item xs={3}>
              ...
              { true && <Tooltip title={`Import Skills`}>
                <Fab size="small" color="primary" style={{marginLeft: "8px"}}
                  onClick={ () => {
                    this.setState({ mode: "import" }, () => {
                      if (this.state.import_skills === null) {
                        this.api.get5eObjects("skills").then((res: any) => {
                          this.setState({ 
                            import_skills: res.results,
                            page_num: 0 
                          });
                        });
                      }
              ...
              { true && <Tooltip title={`Import Proficiencies`}>
                <Fab size="small" color="primary" style={{marginLeft: "8px"}}
                  onClick={ () => {
                    this.setState({ mode: "proficiencies" }, () => {
                      if (this.state.import_skills === null) {
                        this.api.get5eObjects("proficiencies").then((res: any) => {
                          this.setState({ 
                            import_skills: res.results,
                            page_num: 0 
                          });
                  ...
                  <Grid key={key} item container spacing={1} direction="row">
                    <Grid item xs={2}>
                      <Tooltip title={`View details for ${o.name}`}>
                        <Button 
                          ...
                          onClick={ () => {
                            this.setState({ redirectTo:`/beyond/skill/details/${o._id}` });
                          }}>
                            {o.name}
                        </Button>
                      </Tooltip>
                    ...
                    <Grid item xs={1}>
                      <Tooltip title={`Edit ${o.name}`}>
                        <Fab size="small" color="primary" style={{marginLeft: "8px"}}
                          onClick={ () => {
                            this.setState({ redirectTo:`/beyond/skill/edit/${o._id}` });
                          }}>
                          <Edit/>
                        </Fab>
                      </Tooltip> 
      ...
      const page_size = 7;
      const filtered: any[] = this.state.import_skills ? this.state.import_skills.filter(o => 
        (this.state.start_letter === "" || o.name.toUpperCase().startsWith(this.state.start_letter)) && 
              ...
              <Grid item container spacing={1} direction="row">
                ...
                <Grid item xs={2}>
                  <Button 
                    fullWidth variant="contained" color="primary" 
                    onClick={ () => {
                      if (this.state.import_skills) {
                        const import_checked: string[] = [];
                        if (this.state.import_skills.length !== this.state.import_checked.length) {
                          this.state.import_skills.forEach(s => {
                            if (this.state.skills && !this.state.skills.some(s2 => s2.name === s.name)) {
                      ...
                      { (this.state.import_skills && this.state.import_skills.length === this.state.import_checked.length) ? "Deselect All" : "Select All" }
                  </Button>
                </Grid>
                <Grid item xs={4}>
                  Name
              ...
              { filtered_and_paged.map((o, key) => {
                return (
                  <Grid key={key} item container spacing={1} direction="row">
                    <Grid item xs={2}>
                      <Checkbox 
                        checked={ this.state.skills ? (this.state.skills.some(s => s.name === o.name) || (this.state.import_checked.some(s => s === o.url) && !this.state.import_checked.slice(this.state.importing_pos === 0 ? 0 : (this.state.importing_pos - 1)).some(s => s === o.url))) : false } 
                        disabled={true}
                        color="primary" 
                      />
                    </Grid>
    ...
    } else if (this.state.mode === "proficiencies") {
      const formHeight = this.props.height - (this.props.width > 600 ? 198 : 198);
      const page_size = 7;
      const filtered: any[] = this.state.import_skills ? this.state.import_skills.filter(o => 
        (this.state.start_letter === "" || o.name.toUpperCase().startsWith(this.state.start_letter)) && 
