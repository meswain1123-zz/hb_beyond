...
class LineageEdit extends Component<Props, State> {
  ...
  submit() {
    this.setState({ processing: true }, () => {
      this.api.upsertObject("lineage", this.state.obj).then((res: any) => {
        this.setState({ processing: false, redirectTo: "/beyond/lineage" });
      });
    });
  }
  ...
  load_object(id: string) {
    const objFinder = this.state.lineages ? this.state.lineages.filter(a => a._id === id) : [];
  ...
  load() {
    this.setState({ loading: true }, () => {
      this.api.getObjects("lineage").then((res: any) => {
        if (res && !res.error) {
          let { id } = this.props.match.params;
          if (id !== undefined && this.state.obj._id !== id) {
            this.setState({ lineages: res }, () => {
              this.load_object(id);
            });
          } else {
            this.setState({ lineages: res, loading: false });
          }
        }
      });
    });
    ...
    if (this.state.loading || this.state.lineages === null) {
      ...
      return (
        <Grid container spacing={1} direction="column">
          <Grid item>
            <Tooltip title={`Back to Lineages`}>
              <Fab size="small" color="primary" style={{marginLeft: "8px"}}
                onClick={ () => {
                  this.setState({ redirectTo:`/beyond/lineage` });
                }}>
                <ArrowBack/>
              </Fab>
            </Tooltip> 
          ...
          <Grid item>
            <span className={"MuiTypography-root MuiListItemText-primary header"}>
              { this.state.obj._id === "" ? "Create Lineage" : `Edit ${this.state.obj.name}` }
            </span>
          </Grid>
          <Grid item 
            style={{ 
              ...
              <Grid item>
                <FeatureBasesInput 
                  ...
                  onAdd={() => {
                    ...
                    feature_base.parent_type = "Lineage";
...
export default connector(LineageEdit);
