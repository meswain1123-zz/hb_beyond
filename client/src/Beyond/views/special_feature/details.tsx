...
const mapState = (state: RootState) => ({
  // objects: state.app.special_features,
  height: state.app.height,
  width: state.app.width
})

const mapDispatch = {
  // setSpecialFeatures: (objects: SpecialFeature[]) => ({ type: 'SET', dataType: 'special_features', payload: objects }),
  // addSpecialFeature: (obj: SpecialFeature) => ({ type: 'ADD', dataType: 'special_features', payload: obj })
}
...
class SpecialFeatureDetails extends Component<Props, State> {
  ...
  // Loads the editing SpecialFeature into state
          ...
          <Grid container spacing={1} direction="column">
            ...
            <Grid item>
              ...
              <Tooltip title={`Edit ${this.state.obj.name}`}>
                <Fab size="small" color="primary" style={{marginLeft: "8px"}}
                  onClick={ () => {
                    this.setState({ redirectTo:`/beyond/special_feature/edit/${this.state.obj._id}` });
                  }}>
                  <Edit/>
                </Fab>
              </Tooltip> 
...
export default connector(SpecialFeatureDetails);
