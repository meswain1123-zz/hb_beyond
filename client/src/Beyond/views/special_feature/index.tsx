...
import { SpecialFeature } from "../../models";
...
class SpecialFeatureIndex extends Component<Props, State> {
  ...
  render() {
    if (this.state.loading) {
      ...
      const page_size = 7;
      const filtered: any[] = this.state.special_features ? this.state.special_features.filter(o => 
        (this.props.special_feature_type === "Any" || o.type === this.props.special_feature_type) &&
                  ...
                  <Grid key={key} item container spacing={1} direction="row">
                    <Grid item xs={2}>
                      <Tooltip title={`View details for ${o.name}`}>
                        <Button 
                          ...
                          onClick={ () => {
                            this.setState({ redirectTo:`/beyond/special_feature/details/${o._id}` });
                          }}>
                            {o.name}
                        </Button>
                      </Tooltip>
                    ...
                    <Grid item xs={1}>
                      <Tooltip title={`Edit ${o.name}`}>
                        <Fab size="small" color="primary" style={{marginLeft: "8px"}}
                          onClick={ () => {
                            this.setState({ redirectTo:`/beyond/special_feature/edit/${o._id}` });
                          }}>
                          <Edit/>
                        </Fab>
                      </Tooltip> 
