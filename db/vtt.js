  ...
  function gotPlayMaps(playMaps) {
    ...
    function respond(response) {
      ...
      if (pos < playMaps.length) {
        ...
        //   delete t.clear;
        //   t.opacity = 1;
        //   t.ownerID = "5f6a533c52b0d02b481dd0fe";
        // });
        playMap2.masks = [];
        playMap2.lightMasks.forEach(m => {
          m.maskType = "light";
          playMap2.masks.push(m);
        });
        playMap2.darkMasks.forEach(m => {
          m.maskType = "dark";
          playMap2.masks.push(m);
        });
        playMap2.fogMasks.forEach(m => {
          m.maskType = "fog";
          playMap2.masks.push(m);
        });
        playMap2.shadowMasks.forEach(m => {
          m.maskType = "shadow";
          playMap2.masks.push(m);
        });
        playMap2.rainMasks.forEach(m => {
          m.maskType = "rain";
          playMap2.masks.push(m);
        });
    ...
    playMap.shadowMasks.forEach(m => {
      m.maskType = "shadow";
      playMap.masks.push(m);
    });
    playMap.rainMasks.forEach(m => {
      m.maskType = "rain";
      playMap.masks.push(m);
    });
    updatePlayMap(respond, playMap);
...
function AddTokens() {
  const tokens = [
    { 
      name: "WhiteFemaleDragonborn",
      category: "character"
    },
    { 
      name: "WhiteFemaleDragonbornInverted",
      category: "character"
    },
  ];
  let pos = 0;
